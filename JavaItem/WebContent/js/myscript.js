    $(function () {
        ...
        $("#selectPurchase").click(function () {
            ...
            $.ajax({
                url: "RunPage?action=selectPurchase",
                success: function (result) {
                    var data = JSON.parse(result);
                    console.log(data);
                    var $tag = "<br><table border='1'><caption>" + $this + "</caption>";
                    $tag += "<th width=100>구매번호</th><th width=100>라인번호</th><th width=200>상품명</th><th width=80>입고수량</th><th width=80>입고단가</th><th width=150>구매업체</th><th width=80>입고여부</th><th width=80>처리</th>";
                    for (var i = 0; i < data.length; i++) {
                        $tag += "<tr><td>" + data[i].purchaseNo + "</td><td>"
                            + data[i].lineNum + "</td><td>"
                            + data[i].productName + "</td><td>"
                            + data[i].receiptQty + "</td><td>"
                            + data[i].receiptPrice + "</td><td>"
                            + data[i].supplierName + "</td><td>"
                            + data[i].txnYn + "</td><td>"
                            + "<button>수정</button>" + "</td></tr>";
                    }
                    $tag += "</table>";
